// console.log('사각형의 넓이 구하기 (함수 보다 위에서 호출시)', square(10, 8)); // 실행 안됨
// 함수 호이스팅(Hoisting)
// 함수 선언식으로 함수를 만들었을 때는 선언보다 위헤서 호출 가능
// 함수 표현식, 화살표함수는 호출 불가
console.log('사각형 넓이 구하기 (함수 선언식, 함수표현식 보다 위에서 호출시)', myFunc('하이')); // 실행 안됨
...
myFunc('이거 콘솔에 찍히지 않는다'); // console.log() 사용 안했으니까 그냥 냅다 적은것
console.log(myFunc('이렇게 해야 정상적으로 콘솔에 찍힌다'));

// 간단 실습
// 함수를 호출만 해도 콘솔에 찍히게 하기
myFunc2('이렇게 호출만 해도 콘솔에 찍혀야 한다.');
...
const myFunc3 = function () {
    console.log('이렇게 함수 표현식으로 함수를 선언할 수 있다');
}
myFunc3();
